...
# I use the main branch here and you also can use 42.alpha instead.
# These file are not updated since 11 months ago
run tar -xvf ${topdir}/SOURCES/mutter-42.alpha.tar.xz
run wget -nc ${blur_effect_url}/main/src/shell-blur-effect.c
run wget -nc ${blur_effect_url}/main/src/shell-blur-effect.h

run cd mutter-42.alpha
run git init
